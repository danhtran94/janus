			...
			for _, hostName := range def.Hosts {
				for _, method := range def.Methods {
					if strings.ToUpper(method) == methodAll {
						p.router.Any(hostName, listenPath, handler.ServeHTTP, def.middleware...)
					} else {
						p.router.Handle(hostName, strings.ToUpper(method), listenPath, handler.ServeHTTP, def.middleware...)
					}
				}
			}
		} else {
